...
public class JwtService{
    ...
    public Optional<String> extractAccessToken(HttpServletRequest request){
        return Optional.ofNullable(request.getHeader(this.jwtProperty.getAccess().getHeader())).filter(StringUtils::hasText).filter(accessToken -> accessToken.startsWith(jwtProperty.getBearer())).map(accessToken -> accessToken.replace(jwtProperty.getBearer(), ""));
    }
    ...
    public String getUserIdFromToken(String token){
        try{
            return Jwts.parser().setSigningKey(this.jwtProperty.getSecret()).parseClaimsJws(token).getBody().getSubject();
    ...
    public TokenResponse createServiceToken(User users){
        ...
        this.redisService.setDataExpire(String.valueOf(users.getId()), refreshToken, this.jwtProperty.getRefresh().getExpiration());
    ...
    private String createToken(String payload, Long tokenExpiration){
        ...
        return Jwts.builder().setClaims(claims).setIssuedAt(new Date()).setExpiration(tokenExpiresIn).signWith(SignatureAlgorithm.HS512, this.jwtProperty.getSecret()).compact();
    }
